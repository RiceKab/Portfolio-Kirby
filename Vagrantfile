...
Vagrant.configure("2") do |config|
    ...
    # SSL
    # config.vm.provision :shell, :inline => "sed -i '/DocumentRoot/r /vagrant/ssl/ssl_conf.txt' /etc/apache2/sites-available/000-default.conf"
    # config.vm.provision :shell, :inline => "sed -i '/VirtualHost *:80/r /vagrant/ssl/ssl_redirect.txt' /etc/apache2/sites-available/000-default.conf"
    # HEADERS
    config.vm.provision :shell, :inline => "sudo a2enmod headers"
    config.vm.provision :shell, :inline => "sed -i '/DirectoryMatch>/r /vagrant/apache_security/headers.txt' /etc/apache2/conf-available/security.conf"
